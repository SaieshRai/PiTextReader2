...
##### USER VARIABLES
DEBUG = 1  # Enable Debugging to troubleshoot
...
def playTTS():
    ...
    # Debugging: Print text being read
    with open('/tmp/text.txt', 'r') as file:
        text = file.read()
        print("TTS text:", text)  # Debugging output
...
def getData():
    ...
    # Adding psm 3 to improve OCR recognition
    ...
    if os.path.exists('/tmp/text.txt'):
        with open('/tmp/text.txt', 'r') as file:
            ...
            if content.strip():  # Check if the file is not empty
                ...
                speak("Sorry, I couldn't detect any text.")
    else:
        speak("Sorry, an error occurred while reading the text.")
